...
(function() {
    ...
    // ## Get users by group on Jira ##
    // ### Takes ###
    //
    // group: The group of the users to return
    // startAt: The index of the first user to return (0-based)
    // endAt: The index of the last user to return (0-based)
    // Unfortunately, Jira REST API doesn't support maxResults for querying users by group;
    ...
    //
    // *  error: string if there's an error
    // *  users: array of users from the group
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#d2e3756)
    //
    this.getUsersByGroup = function(group, startAt, endAt, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri(
                '/group?groupname=' + group + '&expand=users[' + startAt.toString() + ':' + endAt.toString() + ']'),
            method: 'GET',
            json: true,
            followAllRedirects: true
        };
